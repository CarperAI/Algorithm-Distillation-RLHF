...
def main(hparams={}):
    ...
    def reward_fn(samples: List[str]) -> List[float]:
        ...
        sentiments = list(map(partial(get_score_for_label, 'sadness'), output_batch))
        return sentiments

    stories = load_dataset("adamlin/roc_story")
    format_prompt = lambda d : d['sentence1'] + ' ' + d['sentence2']
    prompts = [format_prompt(d) for d in stories['train']]
    eval_prompts = [format_prompt(d) for d in stories['validation'].select(range(64))]
